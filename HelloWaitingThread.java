...
create snapshot:
`./java -Xint -Xmx64m -Xms8m -Xgcpolicy:optthruput -Xshare:off -Xsnapshot:file=image,trigger=HelloWaitingThread.snapshot -Xthr:deflationPolicy=never -Dcom.ibm.tools.attach.enable=no -cp ~/. HelloWaitingThread`

restore snapshot:
`./java -Xint -Xmx64m -Xms9m -Xgcpolicy:optthruput -Xshare:off -Xrestore:file=image -Xthr:deflationPolicy=never -Dcom.ibm.tools.attach.enable=no -cp ~/. HelloWaitingThread`

*/

public class HelloWaitingThread {
	...
	public static int waitingOnMonitor = 0;
	...
	public static void main(String args[]) throws Throwable {
		...
		synchronized (monitor) {
			...
			/* try to make sure SnapshotThread is waiting on monitor */
			Thread.sleep(500);
...
class InflationThread extends Thread {
	public void run() {
		try {
			synchronized (HelloWaitingThread.monitor) {
				Thread.sleep(500);
			}
		} catch (Throwable e) {
			e.printStackTrace();
...
class SnapshotThread extends Thread {
	public void run() {
		try {
			...
			synchronized (HelloWaitingThread.waitForStart) {
				HelloWaitingThread.waitForStart.notify();
			}
